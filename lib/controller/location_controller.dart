...
class LocationController extends GetxController implements GetxService {
  ...
  Future<void> getAddressList() async {
    ...
    if (response.statusCode == 200) {
      ...
      response.body.forEach((address) {
        ...
        print("Address + ${AddressModel.fromJson(address).id}");
      });
    } else {
  ...
  Future AdvertisePost(id, serviceId, text, AddName, imageFile) async {
      Response response = await locationRepo.postAdvertise(
          id, serviceId, text, AddName, imageFile);
      ...
      //     Get.back();
      //   }
      //   showCustomSnackBar("${response.body}", isError: false);
      // } else {
      //   showCustomSnackBar(
      //       response.body['error_message'] ?? response.bodyString);
      // }
